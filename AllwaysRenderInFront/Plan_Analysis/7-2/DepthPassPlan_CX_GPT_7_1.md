...
## 追加核查：第 2 点 render state 与 Mobile Forward subpass

结论：第 2 点的风险判断成立。`MobileAfterTranslucencyPass` 如果复用 `FMobileBasePassMeshProcessor` 绘制 opaque 颜色，必须强制传入 `ForcePassDrawRenderState`，并把 draw render state 固定为：

- color write：`CW_RGBA`
- depth access：`DepthRead_StencilRead`
- depth state：`TStaticDepthStencilState<false, CF_DepthNearOrEqual>`
- flags：带 `EFlags::ForcePassDrawRenderState`

否则 `FMobileBasePassMeshProcessor::Process` 确实可能把深度比较改成 `CF_Equal`，也可能走 `MobileBasePass::SetOpaqueRenderState` 把 depth write 打开。

### 1. `FMobileBasePassMeshProcessor` 是否会改成 `CF_Equal` / 打开 depth write

代码位置：`Engine/Source/Runtime/Renderer/Private/MobileBasePass.cpp`

`FMobileBasePassMeshProcessor::Process` 中先复制传入的 `PassDrawRenderState`，然后只有在没有 `ForcePassDrawRenderState` 时才会按 base pass 内部逻辑重写状态：
...
同文件 `MobileBasePass::SetOpaqueRenderState` 中，当 `bEnableReceiveDecalOutput || bUsesDeferredShading` 为真时会设置：
...
这里模板第一个参数是 `true`，即 draw PSO 的 depth write 会被打开。因此第 2 点中 “否则可能改成 `CF_Equal` 或打开 depth write” 不是推测，而是当前 `FMobileBasePassMeshProcessor::Process` 的实际分支行为。

### 2. `NextSubpass()` 做了什么

代码位置：`Engine/Source/Runtime/Renderer/Private/MobileShadingRenderer.cpp`

`RenderForwardSinglePass` 中创建 RDG pass 前设置：
...
在同一个 RHI render pass 内先画 opaque/base pass，然后调用：
...
之后才绘制 deferred decals、modulated shadows、fog 和 translucency。

`NextSubpass()` 本身不创建新的深度读写状态，也不会修改 mesh draw command 的 depth compare/write。它只是推进到当前 render pass 已经声明好的下一个 subpass：

代码位置：`Engine/Source/Runtime/RHI/Public/RHICommandList.h`

```cpp
void NextSubpass()
{
	CheckInsideRenderPass();
	RHINextSubpass();
	IncrementSubpass();
}
```

真正的 subpass 拓扑来自 render pass 开始时的 `FRHIRenderPassInfo.SubpassHint`。

### 3. 深度附件读写在哪里确定

移动端 Forward 的 render pass depth/stencil 绑定在 `MobileShadingRenderer.cpp` 中确定。

`InitRenderTargetBindings_Forward` 中：
...
- 有 full depth prepass 时，base render pass 的 depth attachment 是 `DepthRead_StencilWrite`。
- 没有 full depth prepass 时，base render pass 的 depth attachment 是 `DepthWrite_StencilWrite`。

RDG 在执行 raster pass 时把这些绑定转换成 RHI render pass：

代码位置：`Engine/Source/Runtime/RenderCore/Private/RenderGraphPass.cpp`
...
代码位置：`Engine/Source/Runtime/RenderCore/Private/RenderGraphBuilder.cpp`

```cpp
static_cast<FRHICommandList&>(RHICmdListPass).BeginRenderPass(Pass->GetParameters().GetRenderPassInfo(), Pass->GetName());
```

因此附件层面的 depth read/write、load/store、subpass hint 是 RDG render target binding 在 `BeginRenderPass` 时确定的；`FMobileBasePassMeshProcessor` 只决定每个 draw command 的 PSO/render state，例如 blend state、depth compare、depth write、stencil op。

### 4. `scene depth is read only and can be fetched` 的实际含义

`ESubpassHint::DepthReadSubpass` 在 RHI 层表示这个 render pass 会包含深度可读取的后续 subpass：

代码位置：`Engine/Source/Runtime/RHI/Public/RHIResources.h`
...
Vulkan RHI 根据这个 hint 创建第二个 subpass。

代码位置：`Engine/Source/Runtime/VulkanRHI/Private/VulkanRenderpass.h`
...
当 `bDepthReadSubpass` 为真时，Vulkan render pass 会给后续 subpass 添加 depth input attachment，并使用只读深度布局：

```cpp
DepthStencilAttachment.layout = VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL;
DepthInputAttachment = Index;
DepthInputAttachmentLayout = VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL;
```

随后创建 “Color write and depth read sub-pass”：

```cpp
SubpassDesc.SetColorAttachments(ColorAttachmentReferences, 1);
InputAttachments1[InputAttachment1Count].attachment = DepthInputAttachment;
InputAttachments1[InputAttachment1Count].layout = DepthInputAttachmentLayout;
InputAttachments1[InputAttachment1Count].SetAspect(DepthInputAspectMask);
...
并加上从第一 subpass 深度写入到第二 subpass input attachment 读取的依赖：
...
所以 `// scene depth is read only and can be fetched` 指的是：进入下一 subpass 后，硬件/API 的 render pass attachment 布局允许 shader 以 input attachment/subpass fetch 方式读取 scene depth；它不等价于自动把后续 draw 的 PSO depth write 关闭，也不等价于自动把 depth compare 固定为 `CF_DepthNearOrEqual`。

### 5. 对第 2 点的最终确认

第 2 点应保留并加强为以下判断：

1. Subpass / render pass 层面的深度可读写由 `FRenderTargetBindingSlots.DepthStencil` 和 `SubpassHint` 在 RDG `BeginRenderPass` 时确定。
2. Mesh draw command 层面的 `color write`、`depth compare`、`depth write` 由 `FMeshPassProcessorRenderState` 以及 `FMobileBasePassMeshProcessor::Process` 决定。
3. `NextSubpass()` 只切到已经创建好的下一 subpass，不会替后续 draw 修正 depth write 或 depth compare。
4. 因此 `MobileAfterTranslucencyPass` 复用 `FMobileBasePassMeshProcessor` 时，必须带 `ForcePassDrawRenderState`，否则：
   - masked/full prepass 分支可能把 depth compare 改成 `CF_Equal`；
   - opaque render state 分支可能通过 `MobileBasePass::SetOpaqueRenderState` 打开 depth write；
   - 这些 draw state 与 “透明后只补 opaque color、只读 depth、不写 depth” 的 pass 语义不一致。

推荐实现约束不变：`MobileAfterTranslucencyPass` 应显式使用 `CW_RGBA`、`DepthRead_StencilRead`、`TStaticDepthStencilState<false, CF_DepthNearOrEqual>`，并在创建 `FMobileBasePassMeshProcessor` 时传入 `EFlags::ForcePassDrawRenderState`。
