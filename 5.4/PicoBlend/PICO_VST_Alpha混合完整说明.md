...
```cpp
// Engine/PICO-Unreal-Integration-SDK/UE_5.4/Plugins/PICOXR/
// Source/PICOXRHMD/Private/PXR_StereoLayer.cpp:813-848
if (ID == 0)
{
...
> Masked 公式指普通 Masked；Alpha To Coverage 使用独立路径，不在本文范围内。

### 3.2 Translucent 相关代码
...
## 4. 特殊透明混合与 Write Alpha Only

### 4.1 Translucent 在 Clip Mode 中被二次缩放
...
因此 Additive 已写入 EyeBuffer 的 RGB 会被完全裁掉，在 VST 中不可见。切换为 Additive Mode 后：
...
该写入只对通过光栅化、深度/模板测试且未被裁剪的像素生效；重叠区域由最后一次写入决定。
...
| `Before` | 当前材质绘制前的 SceneColor RT |
| `After` | 当前材质绘制后的 SceneColor RT |
| `Eye` | 最终提交给 PICO 的 EyeBuffer |
| `VST` | 视频透视画面 |
| `Screen` | PICO 合成后的屏幕输出 |
| `Target` | 希望最终得到的虚拟颜色强度 |

例如，`C_Before` 表示绘制前 SceneColor RT 的 RGB，`A_Eye` 表示 EyeBuffer 的 Alpha。

### 1.2 EyeBuffer Alpha 的含义
